...
base0 = np.array([0,0.475,0.91110,0.9722,0.99,0.9999,0.9999,0.9999,0.9999])
...
for i in range(8):
    ...
    for j in range(20):
        temp = np.random.uniform(low,high,1)
        generate0.append(temp[0])

generate0 = np.array(generate0)
generate0 = np.sort(generate0)

base00 = np.array([0,0.525,0.94110,0.9822,0.99,0.9999,0.9999,0.9999,0.9999])
...
for i in range(8):
    ...
    for j in range(20):
        temp = np.random.uniform(low,high,1)
        generate00.append(temp[0])

generate00 = np.array(generate00)
generate00 = np.sort(generate00)
...
# plt.title('(b)',y=-0.20)
y1,=plt.plot(np.linspace(0,5,9),base11,'ro-')
...
qq = plt.legend([y1,y0,y00], ["负相关定位模型:ALE=0.77m","变分负相关定位模型:ALE=0.74m","多分布负相关定位模型:ALE=0.69m"], loc='lower right')

plt.savefig("0002.svg")
plt.show()
