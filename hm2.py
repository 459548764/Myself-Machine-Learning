...
def loadDataSet(folder = 'pic_data'):
    trainData = np.zeros([2500,SequenceLength,img_height,img_width,3],dtype='float16')
    trainLabel = []
    testData = np.zeros([1000,SequenceLength,img_height,img_width,3],dtype='float16')
    testLabel = []
    class_name = 0
        ...
        for video_name in os.listdir(folder2):
            ...
            if sample_cnt in sample:
                for video_pic in os.listdir(path):
                    ...
                    sample_len = np.sort(random.sample(range(len(os.listdir(path))),SequenceLength))
                    print(sample_len)
                    cnt = 0
